...
assert_contains "$GLOBAL" '--uc-topbars-combined-height:' "global positioning owns combined toolbar height"
assert_contains "$GLOBAL" 'calc(var(--uc-menubar-height) + var(--uc-navbar-height) + var(--uc-tabbar-height) + var(--uc-bookbar-height));' "combined toolbar height includes every visible toolbar"
...
assert_block_contains "$bottom_fixed_urlbar_block" ':root:has(:is(#urlbar-searchmode-switcher, .searchmode-switcher)[open]) #urlbar' "bottom fixed urlbar handles current and legacy searchmode switchers"
...
assert_contains "$URLBAR" '#urlbar, :root:has(:is(#urlbar-searchmode-switcher, .searchmode-switcher)[open]) #urlbar' "urlbar float searchmode selector remains scoped"
