...
- `qol • 🟡 •` Bookmarks toolbar centered position now only applies when the window is 1920px in width. (ultima.navbar.bookmarks.position = centered) #447
...
- `qol • 🟡 •` Added `ultima.sidebery.indent.standard` alongside `ultima.sidebery.indent.compact` so Sidebery indentation can be switched explicitly in about:config
